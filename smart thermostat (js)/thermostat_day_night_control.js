...
var THERMOSTAT_TARGET_1 = "coap://localhost:5685/apps/running/thermostat_simulation_1/temperature";
var THERMOSTAT_TARGET_2 = "coap://localhost:5685/apps/running/thermostat_simulation_2/temperature"
var MULTICAST = "coap://localhost:5685/apps/running/multicast/tasks";
var MULTICAST_CAST = MULTICAST + "/" + APP_NAME + "/cast";
var TIMED_REQUEST = "coap://localhost:5685/apps/running/timed_action/tasks";

var URL_SUNRISE = "coap://localhost:5685/apps/running/weather_simulation/sunrise"; //"coap://localhost:5685/apps/running/weather/astronomy/sunrise";
var URL_SUNSET = "coap://localhost:5685/apps/running/weather_simulation/sunset"; //"coap://localhost:5685/apps/running/weather/astronomy/sunset";
...
function start() {
	...
	var multicast_payload = "resid = " + APP_NAME + "\n" +
	"target1 = " + THERMOSTAT_TARGET_1 + "\n" + 
	"target2 = " + THERMOSTAT_TARGET_2 + "\n";

	var multicast_request = new CoAPRequest();
	multicast_request.open("POST", MULTICAST);
	multicast_request.async = false;
	multicast_request.send(multicast_payload);
	
	response = multicast_request.responseText; // block
	...
	if (now < dayStart) {
		sendRequest("PUT", MULTICAST_CAST, null, THIS.tempres.nightres.getInfo());
	} else if (now < dayEnd) {
		if (now < sunRise) {
			if (sunRise < dayStart) {
				sendRequest("PUT", MULTICAST_CAST, null, THIS.tempres.dayres.getInfo());
			} else { // sunRise >= dayStart
				sendRequest("PUT", MULTICAST_CAST, null, THIS.tempres.dayres.getInfo() + TEMPERATURE_INCREASE);
			}
		} else if (now < sunSet) {
			sendRequest("PUT", MULTICAST_CAST, null, THIS.tempres.dayres.getInfo());
		} else { // now >= sunSet
...
function stop() {
	...
	var deleteMulticast = new CoAPRequest();
	deleteMulticast.open("DELETE", MULTICAST + "/" + APP_NAME);
	deteteMulticast.send();
}
...
function setDaystart(sunrise) {
	...
	if (sunRise >= dayStart) {		
		if (dayStart > now) {
			...
			postTimedRequest(DAYSTART_TASK, MULTICAST_CAST, "PUT", (THIS.tempres.dayres.getInfo() + TEMPERATURE_INCREASE), dayStart);
		}
		
		if (sunRise > now) {
			app.dump("send timed sunrise");
			
			postTimedRequest(SUNRISE_TASK, MULTICAST_CAST, "PUT", THIS.tempres.dayres.getInfo(), sunRise);
		}
	} else { // runRise < dayStart	
		if (dayStart > now) {
			app.dump("Send timed daystart");
			
			postTimedRequest(DAYSTART_TASK, MULTICAST_CAST, "PUT", (THIS.tempres.dayres.getInfo()), dayStart);
		}
	}
}
...
function setDayend(sunset) {
	...
	if (sunSet <= dayEnd) {
		if (sunSet > now) {
			...
			postTimedRequest(SUNSET_TASK, MULTICAST_CAST, "PUT", (THIS.tempres.dayres.getInfo() + TEMPERATURE_INCREASE), sunSet);
		}
				
		if (dayEnd > now) {
			app.dump("send timed dayend");
			
			postTimedRequest(DAYEND_TASK, MULTICAST_CAST, "PUT", (THIS.tempres.nightres.getInfo()), dayEnd);
		}
	} else { // sunSet > dayEnd	
		if (dayEnd > now) {
			app.dump("send timed dayend");
...
function TempRes(resid) {
		...
		if (now < dayStart) {
			if (sunRise < dayStart) {
				...
				if (sunRise < now) {
					...
					sendRequest("PUT", MULTICAST_CAST, null, newTemp + TEMPERATURE_INCREASE);
					changeTimedRequest(DAYSTART_TASK, "payload", newTemp);
				} else { // sunRise >= now
	...
	this.nightres = new ChangeableInfoRes("night", DEFAULT_NIGHT_TEMP, function(oldInfo, newInfo) {
		...
		if (now < dayStart) {
			...
			sendRequest("PUT", MULTICAST_CAST, null, newTemp);
			...
			if (now < sunRise) {
				...
				if (newDate < sunRise) {
					...
					if (newDate < now) {
						...
						sendRequest("PUT", MULTICAST_CAST, null, THIS.tempres.nightres.getInfo());
						postTimedRequest(DAYSTART_TASK, MULTICAST_CAST, "PUT", THIS.tempres.dayres.getInfo(), newDate);
					}
				}
			}
				...
				if (newDate < sunSet) {
					...
					if (newDate < now) {
						...
						if (now < sunSet) {
							...
							postTimedRequest(DAYEND_TASK, MULTICAST_CAST, "PUT", THIS.tempres.nightres.getInfo(), newDate);
							postTimedRequest(SUNSET_TASK, MULTICAST_CAST, "PUT", THIS.tempres.dayres.getInfo() + 1, sunset);
						} else { // now >= sunSet
